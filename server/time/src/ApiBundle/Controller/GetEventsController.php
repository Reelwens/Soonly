			...
			foreach ($user->getCalendars()->getValues() as $key => $calendar) {
				...
				foreach ( $dateRange as $key2 => $date ){
					...
					if ($event === null ) {
						...
						$data["calendars"][$calendar->getId()][$key2]["attachement"]    = $event->getAttachement()->getId() === null ? -1 : $event->getAttachement()->getId();
						$data["calendars"][$calendar->getId()][$key2]["viewed"]         = $event->getViewed();
						$data["calendars"][$calendar->getId()][$key2]["dayLeft"]        = $event->getDate()->diff($calendar->getEndDate())->days;
						$data["calendars"][$calendar->getId()][$key2]["dayNumber"]      = $event->getDate()->diff($calendar->getCreationDate())->days + 1 ;
			...
			foreach ($user->getCalendarsReceiver()->getValues() as $key => $calendar) {
				...
				foreach ( $dateRange as $key2 => $date ){
					...
					if ($event === null ) {
						...
						$data["calendarsReceiver"][$calendar->getId()][$key2]["attachement"]    = $event->getAttachement()->getId() === null ? -1 : $event->getAttachement()->getId();
